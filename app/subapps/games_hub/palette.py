...
# ---------------------------------------------------------------------------
# Cache — one _Palette instance per theme, rebuilt on theme change.

_cache: "_Palette | None" = None


def _invalidate_cache(_theme: str = "") -> None:
    global _cache
    _cache = None
...
def _connect_theme_signal() -> None:
    try:
        from app.core.theme_manager import theme_manager
        theme_manager.theme_changed.connect(_invalidate_cache)
    except Exception:
        pass
...
class GamePalette:
    """
    Call GamePalette.get() inside paintEvent — returns a cached palette that
    is invalidated automatically when the theme changes.
